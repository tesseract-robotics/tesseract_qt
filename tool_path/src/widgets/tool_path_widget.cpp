  ...
  if (results == 1)
  {
    ...
    if (!working_frame.empty() && working_frame != link_name)
    {
      ...
      if (env_wrapper != nullptr && env_wrapper->getEnvironment()->isInitialized())
      {
        ...
        auto it1 = state.link_transforms.find(tesseract::common::LinkId::fromName(working_frame));
        auto it2 = state.link_transforms.find(tesseract::common::LinkId::fromName(link_name));
