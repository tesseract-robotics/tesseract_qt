...
#define ring_span_lite_VERSION                                                                                         \
  nsrs_STRINGIFY(ring_span_lite_MAJOR) "." nsrs_STRINGIFY(ring_span_lite_MINOR) "." nsrs_STRINGIFY(ring_span_lite_PATCH)
...
#define nsrs_USES_STD_RING_SPAN                                                                                        \
  ((nsrs_CONFIG_SELECT_RING_SPAN == nsrs_RING_SPAN_STD) ||                                                             \
   ((nsrs_CONFIG_SELECT_RING_SPAN == nsrs_RING_SPAN_DEFAULT) && nsrs_HAVE_STD_RING_SPAN))
...
#define nsrs_REQUIRES_0(...) template <bool B = (__VA_ARGS__), typename std::enable_if<B, int>::type = 0>
...
struct default_popper
{
  ...
  T operator()(T& t) const { return std11::move(t); }
};
...
public:
  ...
  bool empty() const nsrs_noexcept { return m_size == 0; }

  bool full() const nsrs_noexcept { return m_size == m_capacity; }

  size_type size() const nsrs_noexcept { return m_size; }

  size_type capacity() const nsrs_noexcept { return m_capacity; }
  ...
  reference front() nsrs_noexcept { return *begin(); }

  const_reference front() const nsrs_noexcept { return *begin(); }

  reference back() nsrs_noexcept { return *(--end()); }

  const_reference back() const nsrs_noexcept { return *(--end()); }

  // iteration:

  iterator begin() nsrs_noexcept { return iterator(0, this); }

  const_iterator begin() const nsrs_noexcept { return cbegin(); }

  const_iterator cbegin() const nsrs_noexcept { return const_iterator(0, this); }

  iterator end() nsrs_noexcept { return iterator(size(), this); }

  const_iterator end() const nsrs_noexcept { return cend(); }

  const_iterator cend() const nsrs_noexcept { return const_iterator(size(), this); }

#if nsrs_RING_SPAN_LITE_EXTENSION

  reverse_iterator rbegin() nsrs_noexcept { return reverse_iterator(end()); }

  reverse_iterator rend() nsrs_noexcept { return reverse_iterator(begin()); }

  const_reverse_iterator rbegin() const nsrs_noexcept { return crbegin(); }

  const_reverse_iterator rend() const nsrs_noexcept { return crend(); }

  const_reverse_iterator crbegin() const nsrs_noexcept { return const_reverse_iterator(cend()); }

  const_reverse_iterator crend() const nsrs_noexcept { return const_reverse_iterator(cbegin()); }
#endif
...
#if nsrs_CPP11_OR_GREATER
  nsrs_REQUIRES_0(std::is_copy_assignable<T>::value) void push_back(value_type const& value) noexcept(
...
#if nsrs_CPP11_OR_GREATER
  nsrs_REQUIRES_0(std::is_move_assignable<T>::value) void push_back(value_type&& value) noexcept(
...
#if nsrs_CPP11_OR_GREATER
  nsrs_REQUIRES_0(std::is_copy_assignable<T>::value) void push_front(T const& value) noexcept(
      (std::is_nothrow_copy_assignable<T>::value))
#else
...
private:
  ...
  size_type normalize_(size_type const idx) const nsrs_noexcept { return idx % m_capacity; }

  reference at_(size_type idx) nsrs_noexcept { return m_data[normalize_(m_front_idx + idx)]; }

  const_reference at_(size_type idx) const nsrs_noexcept { return m_data[normalize_(m_front_idx + idx)]; }

  reference front_() nsrs_noexcept { return *(m_data + m_front_idx); }

  const_reference front_() const nsrs_noexcept { return *(m_data + m_front_idx); }

  reference back_() nsrs_noexcept { return *(m_data + normalize_(m_front_idx + m_size - 1)); }

  const_reference back_() const nsrs_noexcept { return *(m_data + normalize_(m_front_idx + m_size - 1)); }
  ...
  void increment_back_() nsrs_noexcept { ++m_size; }

  void decrement_back_() nsrs_noexcept { --m_size; }

  void increment_front_and_back_() nsrs_noexcept { m_front_idx = normalize_(m_front_idx + 1); }

  void decrement_front_and_back_() nsrs_noexcept { m_front_idx = normalize_(m_front_idx + m_capacity - 1); }
...
public:
  ...
  operator ring_iterator<RS, true>() const nsrs_noexcept { return ring_iterator<RS, true>(m_idx, m_rs); }
#endif

  // access content:

  reference operator*() const nsrs_noexcept { return m_rs->at_(m_idx); }
