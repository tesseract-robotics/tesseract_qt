...
#include <tesseract_qt/kinematic_groups/group_tcps_widget.h>
...
{
  std::shared_ptr<GroupTCPsModel> model;
  QVBoxLayout* layout;
  TreeView* tree_view;
};

GroupTCPsWidget::GroupTCPsWidget(QWidget* parent) : QWidget(parent), data_(std::make_unique<Implementation>())
{
  ctor(ComponentInfo());
}
...
std::shared_ptr<GroupTCPsModel> GroupTCPsWidget::getModel() { return data_->model; }
std::shared_ptr<const GroupTCPsModel> GroupTCPsWidget::getModel() const { return data_->model; }
...
void GroupTCPsWidget::ctor(ComponentInfo component_info)
{
  // Create model
  data_->model = std::make_shared<GroupTCPsModel>(std::move(component_info));
