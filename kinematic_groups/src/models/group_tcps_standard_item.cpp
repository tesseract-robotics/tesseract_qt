                                        ...
                                        const Eigen::Isometry3d& tcp)
{
  ...
  group_tcps_[group_name.toStdString()][tesseract::common::LinkId(tcp_name.toStdString())] = tcp;
}
...
void GroupTCPsStandardItem::removeGroupTCP(const QString& group_name, const QString& tcp_name)
{
  ...
  if (group_it != group_tcps_.end())
  {
    ...
    auto state_it = group_it->second.find(tesseract::common::LinkId(tcp_name.toStdString()));
