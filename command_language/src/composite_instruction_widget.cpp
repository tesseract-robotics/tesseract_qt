...
#include <tesseract_qt/command_language/composite_instruction_widget.h>
...
{
  std::shared_ptr<CompositeInstructionModel> model;
  QVBoxLayout* layout;
  TreeView* tree_view;
};
...
std::shared_ptr<CompositeInstructionModel> CompositeInstructionWidget::getModel() { return data_->model; }
std::shared_ptr<const CompositeInstructionModel> CompositeInstructionWidget::getModel() const { return data_->model; }

QItemSelectionModel& CompositeInstructionWidget::getSelectionModel() { return *data_->tree_view->selectionModel(); }
const QItemSelectionModel& CompositeInstructionWidget::getSelectionModel() const
{
  return *data_->tree_view->selectionModel();
}

void CompositeInstructionWidget::ctor(ComponentInfo component_info)
{
  // Create model
  data_->model = std::make_shared<CompositeInstructionModel>(std::move(component_info));
